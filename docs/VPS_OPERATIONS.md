...
| 2026-09-08 | **Three decisions taken with 9.27's own instrumentation in hand** (PAP 9.29). **(1) The discovery half of the Kalshi rotation was wedged too**, on a key we do not own: unseen series were ordered by the venue's `last_updated_ts` alone, so an unproductive one kept its position forever and the slice re-walked the same head. The attempt cursor now orders that half as well (never-attempted first, then least-recently-attempted). **This expands the observed universe** — ~10,700 unseen series swept at 9 a cycle is ~50 days, first full pass near end of October — so it is declared, not filed as a bug fix. **(2) `max_series_per_sync` 40 → 48**, taken only because `cycle_seconds` answered the open question: 40 series in **61 seconds** against a 3,600-second interval. Gives ~21.5h rotation instead of ~26; 48 requests an hour is ~0.013 req/s against a ceiling of 8, so guardrail 8 is untouched. **(3) The paper-export manifest now carries `rows_sha256`** and `scripts/verify_paper_export.py` exists — Phase 15's acceptance criterion had named a validation script that was never written, and until today a published export was checkable only by counting its lines. Verify any export with `python scripts/verify_paper_export.py docs/paper_exports/<date>.jsonl.gz`; the pre-2026-09-08 files have no digest and the verifier prints SKIP rather than passing quietly. **Watch after this deploy:** the Kalshi tail snapshot round (4,551 markets, ~15–27 min against a 30-minute interval) is the constraint discovery pushes on — 9.26's filter blocks a sports flood, but a large economics/politics discovery could still crowd it. The coverage watchdog and the new `sync rotation` line in `lab status` are where that shows. |
| 2026-09-09 | **Write-lock contention, self-inflicted and now bounded.** `database is locked` errors ran 5–8/day for weeks; on 09-09 they hit **98** — `job_resolutions` 39, `job_kalshi_resolutions` 34, `job_manifold_sync` 19, `job_sync` 18, `job_kalshi_sync` 15. Cause was two-sided. **Mine:** the 09-08 Kalshi watcher change stamps `resolution_checked_ts` per candidate, so a second watcher began issuing up to 200 separate write transactions every 30 minutes. **Pre-existing:** `sync_universe` (Polymarket) held **one** transaction across every upsert in the whole universe, and SQLite permits exactly one writer — so everything else queued behind it and gave up at `busy_timeout` (30s). Fixed on both sides: both resolution watchers now stamp the entire batch in **one** transaction before fetching anything (which also strengthens the round-robin — every candidate rotates before any is fetched, so a mid-cycle crash cannot leave the queue head unrotated), `sync_universe` commits per event, and `sync_kalshi_universe` commits every 200 markets *inside* a series as well as between series (series are no longer small: KXCYCLINGSTAGE alone carries 1,000). Same reasoning as the 250-row batch put into `run_forecasts` on 2026-08-25; these call sites never got it. **Watch:** `grep -c 'database is locked' data/logs/lab.jsonl` — expect a return to single digits per day, and treat a rise as a sign that some job has grown a long transaction again. |
| 2026-09-09 | **Why 361 series with open markets went unwalked — it was the lock regression above, not the rotation.** Worth recording because the obvious suspect was wrong twice. First guess: those series are not returned by the category listing, so they never become candidates. Measured against the live venue — 360 of the 361 **are** in the listing (Sports 3,661 series, Entertainment 2,562, Politics 2,316, Elections 1,735, Economics 804, Climate 385, World 149; 11,500 distinct). Second guess: they sort late. Reproduced the ordering exactly against live candidates — they sit at queue positions **3 to 461, median 271**, i.e. at the very front, keyed on `last_synced_ts` from 2026-08-21. At 39 known slots a cycle the queue reaches position 461 in twelve cycles and twenty-three had run. The actual cause: `known` holds 894 series but only **432** carried a cursor, because `job_kalshi_sync` aborted on `database is locked` **15 times that day** and everything uncommitted in a dying cycle rolls back — each abort discards the rest of that rotation. **The lesson for next time: if series stop being walked, check for aborted cycles before suspecting the ordering.** `grep 'job_kalshi_sync' data/logs/lab.jsonl \| grep -c 'raised an exception'`. The per-200-market commits added the same day mean an aborted cycle now keeps its prefix instead of losing the lot. |
| 2026-09-25 | **Audit found four live failures; all four fixed the same morning.** **(1) Orchestrator OOM-killed hourly since 09-24 17:10** (twelve kills in thirteen hours, each one dropping in-flight snapshot rounds). The weekly `lab report` child had outgrown memory with the 2.1GB db (first OOM 09-20 14:04), crossed its 192h control age on 09-24, and the catch-up — which has no backoff, a failure records no success — restarted it every hour. The out-of-process design says an OOM "lands on the child while collection keeps running", but the child lives in `lab-run.service`'s cgroup and systemd's default `OOMPolicy=stop` stops the whole unit on any OOM kill inside it. Fixed: drop-in `/etc/systemd/system/lab-run.service.d/oom-policy.conf` sets **`OOMPolicy=continue`**; batch children get `oom_score_adj=1000` so the kernel always picks them over the collector; `report` is excluded from catch-up (`runner.NO_CATCH_UP` — the weekly cron still runs it, `lab report` on demand). The report's own memory growth is **not** yet fixed: expect the weekly render to fail until it is, harmlessly now. **(2) `database is locked` 112–168/day** — 84 of 94 Kalshi universe syncs and ~80% of resolution-watcher runs died at busy_timeout. My 2026-09-09 fix did not hold (checked over ten minutes and called done). Root cause: the 08-25 batching bounded analytics write locks by row count, not by time — `run_forecasts` held an open transaction across M3's LLM calls and M5's network requests between 250-row commits, and `run_eval` held one per model through every bootstrap. Now the forecast loop closes any open transaction before each model's `forecast()`, and eval commits each row as written. **(3) Off-host backup stuck since 09-24 03:01** — GitHub refused the weekly 2.14GB `lab.db` ("This repository exceeded its LFS budget") and every nightly ledger increment queued behind that commit. The refusal was only a field in an INFO line, and the `backup` heartbeat was sent regardless. Range repaired by hand (`git reset --soft origin/main`, `git rm --cached data/lab.db`, `data/lab.db` added to the mirror's `.gitignore`, pushed as `0f72142`); `db_enabled: false`; a refused push now logs ERROR; the heartbeat and interval stamps now require a push that actually succeeded; `reference/` (markets that carry forecasts, their events, model_versions — deterministic gzip JSONL, plain git) replaces the db as the weekly metadata backup. **The last full db off the host is the 2026-09-16 push** — a current full copy needs a non-LFS store (object storage), which is an account decision. **(4) Kalshi's third blackout, since 09-18** (8–41 markets/day against ~3,800; 7,914 skipped on stale prices on 09-25): the rounds had outgrown the rate budget at one request per market — ~1,800 liquid markets × 2 requests every 5 min is roughly the whole 8 req/s, starving the ~8,200-market tail. Snapshot rounds now fetch market objects ~100 per request (`/markets?tickers=`, verified field-identical against the live API); any ticker a bulk response omits is fetched singly, as before. First measurement: the liquid round went from ~10 min to **3m37s** for 1,802 markets. |
| 2026-09-25 | **Measured after the bulk-fetch deploy:** the Kalshi tail round ran **07:36:23 → 07:37:06, 43 seconds for 10,452 markets** (9,841 written), against about an hour per round that morning; liquid rounds every ~5 minutes at ~3.5 minutes each; zero bulk failures, 2 single-fetch fallbacks failing (delisted markets still flagged open), zero `database is locked` since the 06:53 deploy, zero OOM kills since 07:00. The weekly `lab report` render completed in a memory-capped scope in ~6 minutes, CLV phase at 459MB (was 700–800MB and dying); the heaviest phase is now `shadow` at 649MB — the next thing to watch. **Also deployed:** PAP 9.31's evaluation changes — `forecasts.days_to_resolution_at_ts` (SCHEMA_VERSION 15, forward-only), stated-horizon strata `_hs_` as primary, realized `_hr_` as a confirmatory-only sensitivity check, the `confirmatory` window, and the Kalshi exclusion windows applied to every Kalshi eval row. Expect tonight's eval to write more rows (one more window per stratum) and every Kalshi number to move — it becomes what the plan said it was. |
| 2026-09-25 | **The heartbeat now reports data health, not only process health.** Every large loss in this project was detected and logged and reached nobody: three Kalshi blackouts (21 of 81 confirmatory days) — the coverage watchdog fired ERROR on day one of the third — and a backup that stopped leaving the host while its heartbeat kept pinging success. Worse, `collector` and `backup` share one `HEARTBEAT_URL` and the collector pings every 5 minutes, so even an honest skipped backup ping signalled nothing. Now two **alarms** live in `meta` (`alarm:coverage`, set/cleared by each nightly forecast job's watchdog; `alarm:backup`, set on the first refused push and cleared by the next push that actually lands). While any alarm is set, the collector's 5-minute ping becomes **`POST <HEARTBEAT_URL>/fail`** with the reason as the body — healthchecks.io records it and puts it in the alert — and if the endpoint has no `/fail` route, nothing is sent at all, so the grace period alerts instead: an alarm state never reports success. **What this means for the operator: the healthchecks check going DOWN no longer only means "collector dead" — open the check's event log for the reason, or run `lab status`, which prints `!! ALARM [name] (heartbeat reporting FAIL): <reason>`.** §12 is unchanged in substance: this code still only emits an outbound ping; the external service does the notifying. Also fixed while testing it: a night with nothing new to commit used to count as "backed up" even when last night's refused commit was still unpushed; `publish` now retries unpushed commits on quiet nights and reports `pushed=True` only when nothing on the host is missing from the remote. |
