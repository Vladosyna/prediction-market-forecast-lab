...
**v2.14** — two defects found by asking what else the sports flood of v2.13 had broken, and one of them turned out not to be a flood problem at all. **The Kalshi universe sync was not diluted, it was pinned.** 837 series are known to this lab, 606 carry open markets, and hours since those 606 were last synced measured **p50 383, p90 770, max 1,199** against a configured rotation of ~26 hours — with three consecutive cycles logging `markets_seen: 0`. The mechanism is a cursor derived from data the walk does not always write: `_series_sync_order` ordered known series by `MAX(last_synced_ts)` over their market rows, while `sync_kalshi_universe` fetches `markets_for_series(status="open")` and upserts only what comes back, so a series with no open markets advanced nothing, stayed the oldest, and — Python's sort being stable — was handed back at the head of every cycle; 231 dead series against ~32 slots pinned the queue completely, and the fetch-failure path had the same shape. A `kalshi_series_sync` table now stamps what the sync **attempted**, for every series it walks, and the rotation orders on that — the identical rule `collect/resolutions.py` adopted in July after the same class of wedge. The known/unseen partition deliberately still derives from the markets table: re-keying it on attempts would drag ~10,700 dormant series into `known`, which is precisely the never-seen-first rotation `_series_sync_order`'s own docstring records shipping and failing, and a test now asserts that invariant. `SCHEMA_VERSION` 13 → 14. Two companion wedges of the same class went with it, because restoring the rotation feeds both: `unresolved_kalshi_markets` had `LIMIT` with no `ORDER BY` and its watcher stamped nothing, and `unresolved_closed_markets` had never been narrowed to Polymarket after Phase 10 — so it was fetching, and stamping `resolution_checked_ts` on, rows from venues Gamma has never heard of (961 Manifold, 48 Kalshi, in a 200-row cycle), which meant the global `oldest_check_age_h` could read healthy on Gamma's stamps while another venue's watcher sat wedged. `lab status` gains what nothing could previously see: `last_synced_ts` had exactly **one** reader in the entire codebase — the sync's own ordering — so a 383-hour stall was invisible by construction; there is now a per-venue watcher block and a Kalshi sync-rotation line, plus `cycle_seconds`, which is load-bearing because a sync overrunning its hour runs late under `max_instances=1` and would silently halve the rotation just restored. PAP 9.27 declares the population effects, and the one that matters is **discovery**: a series unvisited for 383 hours contributes no newly listed market at all, and the size of that omission cannot be quantified from our own data by construction. Two magnitudes from the working analysis were wrong and were corrected against the live database before entering the record (1,009 not 18,079; 48 not 215) — the defects were real, the numbers were not. **Second, the weekly replication export had blocked all publication.** It is a full cumulative re-dump, so it grew 0.67 MB → 115 MB between July and 2026-09-06 and crossed GitHub's 100 MB per-file hard limit; because the pre-receive hook scans every blob in the pushed range, one oversized file refused the entire push and left the **2026-09-06 ledger commitment committed but unpublished** — the state `config.yaml` itself calls worthless — and it failed silently because `gitutil._is_non_fast_forward` matches a divergence rejection, not a hook rejection. From 2026-09-13 the snapshot is `YYYY-MM-DD.jsonl.gz`, written streaming and atomically with `mtime=0` so two exports of identical rows are byte-identical; measured on the 08-30 export, gzip -9 gives **16.5×** (96,133,567 → 5,823,204 bytes), which holds past the freeze at the highest weekly growth rate yet seen. Parquet compresses this ~70× and was rejected anyway: it changes the artifact type §13/§15 committed to, where gzip changes only the container. Everything through `2026-08-30.jsonl` stays plain and stays published — this repo's history *is* the pre-registration record — so the series is split, and the split date is documented where a consumer globbing `*.jsonl` would otherwise take a silent partial read. PAP 9.28 discloses the late publication rather than papering over it, and files one defect left deliberately open: the manifest has no content digest, so a published export is verifiable only by line count, and fixing that needs a deterministic sort scoped to the export alone — never in the shared `resolved_forecast_rows`, whose order feeds an unstable `argsort` that could move the pre-registered confidence sequence with no change in data. No model work remains.

**v2.15** — three decisions rather than defect fixes, each taken only once v2.14's own instrumentation had produced the number that settled it. **The discovery half of the Kalshi rotation was wedged the same way the known half had been**, on a key this project does not own: unseen series were ordered by the venue's `last_updated_ts` alone, so a series that yielded nothing kept its position forever and the slice re-walked the same head instead of sweeping the ~10,700-series pool it exists for. The attempt cursor now orders that half too. No configured policy changes — the 20% discovery budget, category map, exclusions and tiering thresholds are all untouched, and this only makes an existing allocation function — but the *effect* is that markets never previously observed will be, so **PAP 9.29 declares a universe expansion** rather than calling it a bug fix: ~9 discovery slots an hour sweep the pool in roughly 50 days, first full pass near end of October against a 2026-12-31 freeze, most of it dormant (the venue lists ~10,500 series against the ~285 that carried open markets), and newly discovered markets are forward-only so discovery date is a covariate rather than a stationary population. **`max_series_per_sync` 40 → 48** is the second, and it is exactly the decision v2.12 deferred five times over: it was held back from v2.14 because a politeness choice must not ride inside a defect fix, and it is taken now because `cycle_seconds` — added in v2.14 precisely to answer this — measured **61 seconds for 40 series against a 3,600-second interval**. Rotation goes ~26h → ~21.5h at ~0.013 req/s against a ceiling of 8. A third correction moved v2.14's own arithmetic: the first post-fix cycle logged `series: 40` but stamped **33** distinct series, because a series returned under more than one configured category was fetched twice — so the cap was budgeting slots, not series, and the "~26-hour rotation" 9.27 asserts was really ~31 until candidates were deduplicated. **Third, the paper export became verifiable.** Phase 15's acceptance criteria name "the `--paper` export round-trips through a validation script"; no such script existed, and no manifest field hashed the content, so every published export was checkable only by counting its lines — a gap v2.14 filed openly rather than fixing in the same breath as a size change. The manifest now carries `rows_sha256` over the newline-joined canonical rows (the convention `docs/ledger_commitments.jsonl` already uses), the export's row order is deterministic so that digest reproduces, and `scripts/verify_paper_export.py` — stdlib only, importing nothing from `src/lab`, because a verifier sharing code with the writer verifies less than it appears to — checks parse, field set, row count and digest. The ordering deliberately lives in the export alone: `resolved_forecast_rows` is shared with `run_eval`, whose cluster ordering uses an **unstable** `np.argsort(resolved_ts)`, so an `ORDER BY` there would move the pre-registered anytime-valid confidence sequence with no change in data — a test now asserts that query stays unordered. What the digest proves is stated where a reviewer will read it: the file is the file that was written, **not** that the rows are immutable, since `event_id`, `tier` and `category` are re-derived from the live markets table on every dump. The same audit corrected a schema-doc line that had claimed `tier` was recorded "at forecast time", which the join does not support. No model work remains.

**v2.16** — an audit on 2026-09-25 found four live failures and a set of research-validity defects, and closed both the same day; none of it is model work. **Live failures.** (1) Kalshi was in its **third blackout** since 09-18 (8–41 markets/day against ~3,800; 7,914 skipped on stale prices) because snapshot rounds cost one request per market and the universe expansion of 9.29 had pushed demand past the 8 req/s budget — ~1,800 liquid markets × 2 requests every 5 minutes starving a ~10,000-market tail. Rounds now fetch market objects ~100 per request (`/markets?tickers=`, verified field-identical live): the tail round went from about an hour to **43 seconds**, the liquid round from ~10 to ~3.5 minutes, with no population change and guardrail 8 untouched. PAP 9.30 declares 09-18..09-25 an exclusion window — Kalshi has lost 21 of 81 confirmatory days — and corrects 9.29, which expanded the universe without bounding it by the capacity to observe it. (2) The orchestrator was **OOM-killed hourly** from 09-24 17:10: the weekly report child had outgrown memory, the catch-up (which has no backoff) re-ran it every hour, and systemd's default `OOMPolicy=stop` turned each child OOM into a full stop — the out-of-process isolation the code relied on had never existed. `OOMPolicy=continue`, `oom_score_adj=1000` for batch children, `report` excluded from catch-up, and the report's own allocation found from its phase log (the CLV snapshot read, not the wealth plots first suspected) and scored market-chunk by market-chunk with provably identical results. (3) **`database is locked` 112–168/day**, failing 89% of Kalshi syncs: v2.12's batching bounded analytics write locks by row count, not time — the forecast pass held a transaction across M3's LLM calls and M5's network requests, eval across every bootstrap for a model. Neither now holds one across slow work; v2.14's lock fix, verified over ten minutes and called done, had not held. (4) The **off-host backup had stopped** on 09-24: the repo exceeded its LFS budget, the refused 2.14GB `lab.db` commit blocked every nightly increment behind it, the refusal was a field in an INFO line, and the backup heartbeat was sent regardless. Range repaired, `lab.db` off LFS (last full copy off-host: 09-16 — a non-LFS store is an account decision), a plain-git `reference/` dump of the rows needed to read the ledger, and every "backed up" marker now requires a push that landed. **Research validity.** H1's horizon strata used the realized horizon, which conditions on the outcome ("by date" markets resolve early when YES): the ≥30-day bucket showed price minus outcome +0.026 realized against −0.009 stated, a miscalibration manufactured in H1's predicted direction. Strata now use the stated horizon, frozen per forecast (`days_to_resolution_at_ts`, as §6 always asked), with realized kept as a sensitivity check (PAP 9.31); the active M1 curve was already fit on the stated horizon. The confirmatory window of §6, the Kalshi exclusion windows of 9.17/9.26/9.30, and seven pre-registered robustness checks (`lab eval --robustness`) existed only as prose and are now code, written before the confirmatory results are read. The CS no longer depends on SQLite's row order (9.32). `resolved_ts` was always the lab's recording time; the venue's own resolution time is now kept too. Polymarket clustered only negRisk events; every multi-market Gamma event is now one cluster — 1,690 clusters were 1,355, and a code comment claiming the grouping vanishes on close was wrong (9.33). With all of that, **H1's primary stratum is ~150 event clusters**, below the plan's own 200 floor, and 9.31 commits to reporting it at the freeze as a bound. The null control was one pooled draw of 30 in which Kalshi's 42,222 sports listings left Polymarket an expected 0.66 markets; it is now 150 per venue (9.34, which also retracts the audit's own permutation-test proposal as an invalid null). **The systemic lesson** — every loss above was detected, logged and read by nobody — is now wired into the one channel §12 allows: data-health alarms turn the collector's heartbeat into a healthchecks `/fail` carrying the reason. No model work remains.
